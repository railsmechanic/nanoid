defmodule Nanoid.NonSecureTest do
  ...
  describe "generate_with/1" do
    ...
    test "with a multi-byte unicode alphabet keeps graphemes intact" do
      alphabet = "äöü🚀🎉"
      nanoid = Nanoid.NonSecure.generate_with(size: 12, alphabet: alphabet)
    ...
    test "raises ArgumentError for an alphabet with fewer than two symbols" do
      assert_raise ArgumentError, fn -> Nanoid.NonSecure.generate_with(alphabet: "a") end
      # "ä" is a single grapheme even though byte_size/1 is 2 — must still raise.
      assert_raise ArgumentError, fn -> Nanoid.NonSecure.generate_with(alphabet: "ä") end
      assert_raise ArgumentError, fn -> Nanoid.NonSecure.generate_with(alphabet: "") end
    end

    test "raises ArgumentError for a non-positive or non-integer size" do
      assert_raise ArgumentError, fn -> Nanoid.NonSecure.generate_with(size: 0) end
      assert_raise ArgumentError, fn -> Nanoid.NonSecure.generate_with(size: -5) end
      assert_raise ArgumentError, fn -> Nanoid.NonSecure.generate_with(size: "10") end
    end
  end
end
